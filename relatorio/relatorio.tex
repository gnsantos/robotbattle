...
\begin{tikzpicture}[>=stealth',shorten >=1pt,node distance=3cm,on grid,initial/.style    ={}]
  \node[state]          (P)                         {$MkOp$};
  \node[state]          (B) [ left =of P]           {$Run$};
  \node[state]          (C) [left =of B]            {$Cons$};
  \node[state]          (D) [below =of C]            {$Pars$};
  \node[state]          (F) [below right   =of P]    {$MkSys$};
  \node[state]          (M) [below left =of F]     {$SysC$};
  \node[state]          (G) [below left =of P]           {$nxtI$};
  \node[state]          (R) [below left =of G]           {$endE$};
\tikzset{mystyle/.style={->,double=orange}} 
\tikzset{every node/.style={fill=white}} 
\path (C)     edge [mystyle]    node   {$1$} (D)
      (C)     edge [mystyle]    node   {$2$} (B)
      (B)     edge [mystyle]    node   {$3$} (P)
      (P)     edge [mystyle]    node   {$4'$} (F)
      (P)     edge [mystyle]    node   {$4$} (G)
      (G)     edge [mystyle]    node   {$5$} (B)
      (G)     edge [mystyle]    node   {$6'$} (F)
      (F)     edge [mystyle]    node   {$7'$} (M)
      (M)     edge [mystyle]    node   {$8'$} (R);
\end{tikzpicture}
\end{center}
\textbf{\text{  Fluxo de processamento da maquina virtual - Legenda }}
\begin{enumerate}
\item[-]{Cons - Construtor do objeto Virtual Machine}
\item[-]{Pars - Parser do arquivo de entrada para o vetor de instruções}
\item[-]{Run - Ciclo de execução do vetor de instruções}
\item[-]{MkOp - Realizar operação sobre as instruções}
\item[-]{nxtI - Chama a próxima instrução do vetor}
\item[-]{MkSys - Instancia uma nova chamada ao sistema}
\item[-]{SysC - Faz uma chamada ao sistema passando como parâmetro a requisição}

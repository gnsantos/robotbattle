...
public class Calc implements CalcConstants {
    ...
    public static String parse(String pathToProg, String pathToDefault){
                initParser();
                InputStream s = null;
                InputStreamReader isr = null;
                BufferedReader buffer = null;
                try{
                        s = new FileInputStream(pathToProg);
                        isr = new InputStreamReader(s);
                        buffer = new BufferedReader(isr);
                } catch(Exception e){
                        System.out.println("File not found. Using default.");
                        try{
                                s = new FileInputStream(pathToDefault);
                        }catch(Exception ex) {System.out.println("Erro fatal. Encerrando programa"); System.exit(1);}
                        isr = new InputStreamReader(s);
                        buffer = new BufferedReader(isr);
                }
                Calc.ReInit(buffer);
                try{
                        Prog = Calc.Programa();
                } catch(Exception parseEx){System.out.println("Erro na compilacao high-level."); System.exit(1);}
                return C.Dump(Prog);
        }
    ...
    public static void main(String args[]) throws ParseException {
        String s;
        initParser();
        Calc.ReInit(System.in);
        Prog = parser.Programa();          // compila e retorna o código de maquina
        s = C.Dump(Prog);                      // manda a string com codigo para s
        System.out.println(s); //imprime o codigo
...
// No nivel mais baixo calcula as expressoes lógicas
  ...
  static final public Vector<Instrucao> SysCall() throws ParseException {
    Vector<Instrucao> p = new Vector<Instrucao>(0);
    Vector<Instrucao> x = new Vector<Instrucao>(0);
    Token t;
    Token u;
    jj_consume_token(17);
    u = jj_consume_token(CHAMADA);
    if (jj_2_17(2)) {
      t = jj_consume_token(DIRECAO);
    } else if (jj_2_18(2)) {
...
// Um item a ser impresso
  ...
  static private boolean jj_3_17() {
    if (jj_scan_token(DIRECAO)) return true;
    return false;
  }
  ...
  static private boolean jj_3R_19() {
    ...
    if (jj_scan_token(CHAMADA)) return true;
    return false;
  }
